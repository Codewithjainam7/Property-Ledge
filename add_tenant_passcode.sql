-- Run this in your Supabase SQL Editor to add the passcode column for OTP verification
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS passcode VARCHAR(10);

-- Update the RPC to include passcode
...
-- Ensure accept clears the passcode
...
BEGIN
  ...
  -- Update tenant: link user_id, activate, and clear token and passcode
